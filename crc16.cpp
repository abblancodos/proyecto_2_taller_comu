...
uint16_t CRC16::compute(const uint8_t *data, std::size_t length) {
...
bool CRC16::verify(const uint8_t *data, std::size_t length) {
  if (length < 2) {
    return false;
  }

  // Compute CRC over data excluding last 2 bytes
  uint16_t computed_crc = compute(data, length - 2);

  // Extract stored CRC (big-endian)
  uint16_t stored_crc = (static_cast<uint16_t>(data[length - 2]) << 8) |
