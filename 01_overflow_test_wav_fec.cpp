...
int main(int argc, char** argv) {
    ...
    std::cout << "bits_enc.size() = " << bits_enc.size() << "\n";
    ...
    std::cout << "bits_dec.size() = " << bits_dec.size() << "\n";

    // Aseguramos que usamos SOLO la parte común (por si algo quedó más corto)
    std::size_t n_info_bits = std::min(bits_in.size(), bits_dec.size());

    // Redondeamos a múltiplo de 8 para reconstruir bytes
    std::size_t n_info_bits_trunc = (n_info_bits / 8) * 8;

    if (n_info_bits_trunc == 0) {
        std::cerr << "Muy pocos bits para reconstruir algo coherente.\n";
        return 1;
    }

    // --- Calcular BER entre bits_in y bits_dec ---
    BerCounter ber;
    ber.update_block(bits_in, bits_dec, n_info_bits_trunc);

    std::cout << "BER = " << ber.ber()
              << " (" << ber.errors() << " / " << ber.total() << ")\n";

    // --- bits -> bytes ---
    std::vector<uint8_t> bits_dec_trunc(bits_dec.begin(),
                                        bits_dec.begin() + n_info_bits_trunc);
    ...
    // Si por alguna razón bytes_out < bytes_in, el resto queda como estaba (0)
